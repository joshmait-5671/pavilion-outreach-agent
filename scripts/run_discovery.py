#!/usr/bin/env python3
"""Discover and score new podcasts for a campaign."""
...
@click.option("--dry-run", is_flag=True, help="Score but do not write to DB or Sheet")
@click.option("--limit", default=None, type=int, help="Process at most N search queries")
@click.option("--min-score", default=None, type=int, help="Override campaign min_score")
def main(campaign, campaigns_dir, db_path, dry_run, limit, min_score):
    """Find new podcasts, score them, add to tracking sheet."""
    ...
    from src.workflow.orchestrator import run_discovery_phase
    import anthropic
